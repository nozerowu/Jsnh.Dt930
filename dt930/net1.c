    ...
    /*memcpy(filename,"data.txt",10);*/
      ...
      while(1)
      {
            ...
            while(1)
              {
                lcd_cls();
                lcd_csr_put( 3, 0 );
                cputstr(LCD_ATTR_NORMAL,"请扫描电脑名称:");
                /*lcd_csr_put( 6, 0 );
                cputstr(LCD_ATTR_NORMAL,"扫描或者输入:");*/
                memset(dnname,0x0,10);
                i=read_str(0,5,0,dnname,10); 
                if(i==1)
                continue;
                if(i==E_KEY_CLR) 
                    return;
             ...
             while(1)
              {
                ...
                lcd_csr_put( 3, 0 );
                memset(dnbar,0x0,32);
                cputstr(LCD_ATTR_NORMAL,"请扫描主机条码:");
            ...
            while(1)
              {
                lcd_cls();
                lcd_csr_put( 3, 0 );
                cputstr(LCD_ATTR_NORMAL,"请输入显示器大小:");
                memset(dissize,0x0,3);
                i=read_str(0,5,2,dissize,3); 
                if(i==1)
                continue;
                if(i==E_KEY_CLR) 
                    return;
                if((strlen(dissize)==0)||(strlen(dissize)>2))
                    continue;
                break; /*若是输入正常则跳出这个循环*/
               }
             ...
             while(1)
              {
                lcd_cls();
                lcd_csr_put( 3, 0 );
                memset(disbar,0x0,32);
                cputstr(LCD_ATTR_NORMAL,"请扫描显示器条码:");
                i=read_str(0,5,0,disbar,32);
                if(i==1)
                continue;
                if (strlen(disbar)==0)
                {
        ...
        /**/
                ...
                lcd_csr_put( 7, 0 );
                 cputstr(LCD_ATTR_NORMAL,"保存[ENT] 放弃[CLR]");
                j=getkey();
                if(j==0x0d) /*OK*/
                  {
                    memset(allbind,0x0,100);
                    memcpy(allbind,oper,6);
                    strcat(allbind,splitdh);
                    strcat(allbind,dnname);
                    strcat(allbind,splitdh);
                    strcat(allbind,dnbar);
                    strcat(allbind,splitdh);
                    strcat(allbind,dissize);
                    strcat(allbind,splitdh);
                    strcat(allbind,disbar);
